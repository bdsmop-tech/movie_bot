...
async def current_items(room: Room):
    # жанры из CSV
    gids = [int(x) for x in room.genre_ids.split(",") if x.strip().isdigit()]
    items = discover(
...
async def send_card(ctx_msg, room: Room):
...
async def start_swipe(msg: types.Message, regexp: types.MessageText):
...
async def on_vote(c: types.CallbackQuery):
    ...
    if await both_voted(room_id, item_id):
        # матч? если оба yes — отправим уведомления
        # Для простоты: при двух yes — напишем участникам (через чат)
        async with async_session() as s:
            rq = await s.execute(select(Room).where(Room.id==room_id))
            room = rq.scalar_one_or_none()
            if room:
                await c.message.answer("🔥 Матч! Оба выбрали 👍")
                # двигаем индекс на следующую карточку
                await s.execute(
                    update(Room).where(Room.id==room_id).values(idx=room.idx+1)
                )
                await s.commit()
